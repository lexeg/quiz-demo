...
DO $EF$
BEGIN
    IF NOT EXISTS(SELECT 1 FROM "__EFMigrationsHistory" WHERE "MigrationId" = '20240506060454_InitialCreate') THEN
...
BEGIN
    IF NOT EXISTS(SELECT 1 FROM "__EFMigrationsHistory" WHERE "MigrationId" = '20240506060454_InitialCreate') THEN
    CREATE TABLE "PresignedUrls" (
        "Id" uuid NOT NULL,
        "BranchOfficeId" uuid NOT NULL,
        "EducationalProgramId" uuid NOT NULL,
        "TestId" uuid NOT NULL,
        "PresignedUrl" text,
        "ExpiredDate" timestamp with time zone NOT NULL,
        CONSTRAINT "PK_PresignedUrls" PRIMARY KEY ("Id"),
        CONSTRAINT "FK_PresignedUrls_branch_office_table_BranchOfficeId" FOREIGN KEY ("BranchOfficeId") REFERENCES branch_office_table (id) ON DELETE CASCADE,
        CONSTRAINT "FK_PresignedUrls_educational_program_table_EducationalProgramId" FOREIGN KEY ("EducationalProgramId") REFERENCES educational_program_table (external_id) ON DELETE CASCADE,
        CONSTRAINT "FK_PresignedUrls_tests_table_TestId" FOREIGN KEY ("TestId") REFERENCES tests_table (id) ON DELETE CASCADE
    );
    END IF;
END $EF$;
...
BEGIN
    IF NOT EXISTS(SELECT 1 FROM "__EFMigrationsHistory" WHERE "MigrationId" = '20240506060454_InitialCreate') THEN
    CREATE INDEX "IX_PresignedUrls_BranchOfficeId" ON "PresignedUrls" ("BranchOfficeId");
    END IF;
END $EF$;
...
BEGIN
    IF NOT EXISTS(SELECT 1 FROM "__EFMigrationsHistory" WHERE "MigrationId" = '20240506060454_InitialCreate') THEN
    CREATE INDEX "IX_PresignedUrls_EducationalProgramId" ON "PresignedUrls" ("EducationalProgramId");
    END IF;
END $EF$;
...
BEGIN
    IF NOT EXISTS(SELECT 1 FROM "__EFMigrationsHistory" WHERE "MigrationId" = '20240506060454_InitialCreate') THEN
    CREATE INDEX "IX_PresignedUrls_TestId" ON "PresignedUrls" ("TestId");
    END IF;
END $EF$;
...
BEGIN
    ...
    VALUES ('20240506060454_InitialCreate', '8.0.3');
    END IF;
END $EF$;
COMMIT;
